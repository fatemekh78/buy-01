...
@Component({
    ...
    styleUrls: ['./my-stats.component.scss'] // Updated to SCSS
})
export class MyStatsComponent implements OnInit {
    ...
    get formattedTotalSpent(): string { return this.userProfile ? `$${this.userProfile.totalSpent.toFixed(2)}` : '$0.00'; }
    get bestCategoryDisplay(): string { return this.userProfile?.mostBoughtCategory || 'Not available'; }
    get lastOrderDisplay(): string {
        if (!this.userProfile?.lastOrderDate) return 'No orders yet';
        return new Date(this.userProfile.lastOrderDate).toLocaleDateString();
    }

    get formattedRevenue(): string { return this.sellerProfile ? `$${this.sellerProfile.totalRevenue.toFixed(2)}` : '$0.00'; }
    get joinDateDisplay(): string {
        if (!this.sellerProfile?.joinDate) return 'Recently joined';
        return new Date(this.sellerProfile.joinDate).toLocaleDateString();
    }
    get lastSaleDateDisplay(): string {
        if (!this.sellerProfile?.lastSaleDate) return 'No sales yet';
